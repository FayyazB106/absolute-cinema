...
export default function DashboardHeader() {
    ...
    return (
        <nav className="db-header shadow-lg sticky top-0 z-50 transition-colors duration-300">
            <div className="w-full px-8 flex flex-row justify-between items-center py-4 gap-4">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`relative w-20 h-12 flex items-center rounded-full p-1 cursor-pointer transition-colors duration-300 shadow-inner
                            ${isDark ? 'bg-slate-700' : 'bg-blue-400'}`}
                        title={t("dashboardHeader.darkMode")}
                    >
                        <div
                            className={`flex items-center justify-center bg-white w-10 h-10 rounded-full shadow-lg transform transition-transform duration-500 ease-in-out
                                ${isDark ? 'translate-x-8 rotate-[360deg]' : 'translate-x-0 rotate-0'}`}
                        >
                            {isDark
                                ? (<Moon className="text-slate-700 animate-in fade-in zoom-in duration-300 flex-shrink-0" />)
                                : (<Sun className="text-yellow-500 animate-in fade-in zoom-in duration-300 flex-shrink-0" />)
                            }
                        </div>
                    </button>
